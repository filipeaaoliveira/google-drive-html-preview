...
Run before every release, in Chrome and in Brave with default shields.
...
| 13a | **Back to Drive** on that "already been opened" page | Navigates to Drive's root, since this path carries no file id, and does not dead-end |
...
| 20 | Preview a video, close it, then open an HTML file in the same session | Renders the HTML file. The video's closed dialog stays in the DOM and must never win |
...
Not yet run. This matrix needs a browser and a Google account, and nothing below
has been executed.
